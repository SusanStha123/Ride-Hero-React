...
import { AdminDriver } from './Helper';
import { FaEnvelope, FaUser, FaUserFriends } from "react-icons/fa";

const Driver = () => {
  const adminDriver = React.useMemo(() => new AdminDriver(), []);
  // const [open, setOpen] = useState(true);
  const [driver, setDriver] = useState([]);

  const getDriver = React.useCallback(async () => {
    let response = await adminDriver.getDriver();
    if (response !== false) {
      setDriver(response)
    }
  }, []);
  ...
  return (
    <>
      <div className='p-5 h-screen bg-gray-100'>
        <h1 className='text-xl mb-4 font-semibold text-primary'>Registered Driver</h1>
                ...
                driver.map((res, index) => {
                  return (
                    <tr className='border-b'>
                      ...
                      <td className='p-3 text-sm font-medium text-center whitespace-nowrap'>{res.status === 1 ? "Passenger" : "Driver"}</td>
                    </tr>
                  )
                })
              }
                    ...
                    <div className='mb-2'>
                      <span className='flex items-center'>
                        ...
                        <div>
                          ...
                          <span> {res.roles === 1 ? "Passenger" : "Driver"}</span>
                        </div>
                      </span>
                    </div>
                  </div>
...
{/* <h1 className='text-2xl font-semibold mb-3 text-primary tracking-wide'>Register Driver Table</h1>
